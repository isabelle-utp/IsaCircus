...
[pred]: "hide\<^sub>r P E = (\<exists> s. (P\<lbrakk>tr\<^sup>< @ \<guillemotleft>s\<guillemotright>,(\<guillemotleft>E\<guillemotright>\<union>ref\<^sup>>)/tr\<^sup>>,ref\<^sup>>\<rbrakk> \<and> $tr\<^sup>> = $tr\<^sup>< @ (\<guillemotleft>s\<guillemotright>\<restriction>\<guillemotleft>-E\<guillemotright>)))\<^sub>e"
...
lemma hide_rea_csp_enable [rpred]: 
  "hide\<^sub>r \<E>(s, t, E) F = \<E>(s \<and> E - \<guillemotleft>F\<guillemotright> = E, t \<restriction> \<guillemotleft>-F\<guillemotright>, E)"
  by (pred_auto)

lemma hide_rea_csp_do [rpred]: "hide\<^sub>r \<Phi>(s,\<sigma>,t) E = \<Phi>(s,\<sigma>,t \<restriction> \<guillemotleft>-E\<guillemotright>)"
  by (pred_auto)

lemma hide_rea_seq [rpred]:
  assumes "P is CRR" "$ref\<^sup>> \<sharp> P" "Q is CRR" 
  shows "hide\<^sub>r (P ;; Q) E = hide\<^sub>r P E ;; hide\<^sub>r Q E"
proof -
  have"hide\<^sub>r (CRR(\<exists> ref\<^sup>> \<Zspot> P) ;; CRR(Q)) E = hide\<^sub>r (CRR(\<exists> ref\<^sup>> \<Zspot> P)) E ;; hide\<^sub>r (CRR Q) E"
...
lemma hide_rea_true_R1_true [rpred]: 
  ...
  by (pred_auto, metis append_Nil2 seq_filter_Nil)
...
lemma st'_unrest_hide_rea [unrest]: "$st\<^sup>> \<sharp> P \<Longrightarrow> $st\<^sup>> \<sharp> hide\<^sub>r P E"
  by (pred_auto, blast+)

lemma ref'_unrest_hide_rea [unrest]: "$ref\<^sup>> \<sharp> P \<Longrightarrow> $ref\<^sup>> \<sharp> hide\<^sub>r P E"
  by (pred_auto, blast+)
...
lemma abs_rea_false [rpred]: "abs\<^sub>r false E = false"
  by (pred_simp, metis append.right_neutral seq_filter_Nil)
...
lemma hide_rea_impl_under_abs:
  ...
  shows "(abs\<^sub>r P A \<longrightarrow>\<^sub>r hide\<^sub>r (P \<longrightarrow>\<^sub>r Q) A) = (abs\<^sub>r P A \<longrightarrow>\<^sub>r hide\<^sub>r Q A)"
  by (simp add: RC1_def abs_rea_def rea_impl_def rpred closure assms unrest)
     (pred_auto, metis order_refl )
...
proof -
  have "?lhs = \<^bold>R\<^sub>s (abs\<^sub>r P A \<turnstile> hide\<^sub>r (P \<longrightarrow>\<^sub>r Q) A \<diamondop> hide\<^sub>r (P \<longrightarrow>\<^sub>r R) A)"
    by (simp add: HideCSP_def rdes assms closure)
  also have "... = \<^bold>R\<^sub>s (abs\<^sub>r P A \<turnstile> (abs\<^sub>r P A \<longrightarrow>\<^sub>r hide\<^sub>r (P \<longrightarrow>\<^sub>r Q) A) \<diamondop> (abs\<^sub>r P A \<longrightarrow>\<^sub>r hide\<^sub>r (P \<longrightarrow>\<^sub>r R) A))"
    by (meson rea_impl_mp srdes_tri_eq_intro)
  also have "... = ?rhs"
    by (metis (no_types, lifting) assms(1,2,3) hide_rea_impl_under_abs rea_impl_mp srdes_tri_eq_intro)
  finally show ?thesis .
qed
...
lemma HideCSP_DoCSP [rdes_def]: 
  "do\<^sub>C(a) \\\<^sub>C A = (Skip \<triangleleft> (a \<in> \<guillemotleft>A\<guillemotright>) \<triangleright>\<^sub>R do\<^sub>C(a))"
  by (rdes_eq)

lemma HideCSP_PrefixCSP:
  assumes "P is NCSP"
  shows "(a \<rightarrow>\<^sub>C P) \\\<^sub>C A = ((P \\\<^sub>C A) \<triangleleft> (a \<in> \<guillemotleft>A\<guillemotright>) \<triangleright>\<^sub>R (a \<rightarrow>\<^sub>C (P \\\<^sub>C A)))"
