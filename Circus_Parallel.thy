...
lemma lens_override_reorder:
  assumes "X \<bowtie> Y"
  shows "(S\<^sub>1 \<oplus>\<^sub>L S\<^sub>2 on X) \<oplus>\<^sub>L S\<^sub>3 on Y = (S\<^sub>1 \<oplus>\<^sub>L S\<^sub>3 on Y) \<oplus>\<^sub>L S\<^sub>2 on X"
  using assms
  by (simp add: lens_override_def lens_indep.lens_put_comm)
...
proof -
  have "?lhs = P \<parallel>\<^bsub>N\<^sub>C ns2 cs ns1\<^esub> \<E>(s\<^sub>0,t\<^sub>0,E\<^sub>0)"
    by (simp add: CSPInnerMerge_commute assms)
  also from assms have "... = ?rhs"
    find_theorems lens_override lens_indep
    apply (simp add: merge_csp_enable_right lens_indep_sym assms(3) tr_par_sym)
    apply (simp add: lens_override_reorder assms(3))
    oops
...
translations
  ...
  "_par_ns_csp P ns1 cs ns2 Q" == "P \<parallel>\<^bsub>M\<^sub>C ns1 cs ns2\<^esub> Q"
